...
export default {
  ...
  setup(props) {
    ...
    function loadCompanyDetails() {
      let seedData
      var totalCompanyAccountNumber = 0
      companyInfoList.value.forEach((element) => {
        totalCompanyAccountNumber = element.NumberofAccounts + totalCompanyAccountNumber
      })
      ...
      var width = isXsOnly.value ? divWidth.offsetWidth : smOnly.value ? 280 : 320
      var height = 200
      ...
      if (width > 1920) {
        transformWidth = 180
      } else if (width < 1600 && width > 1399) {
        transformWidth = 195
      } else if (width < 1400 && width > 1300) {
        transformWidth = 195
      } else if (width < 1400 && width > 1250) {
        transformWidth = 160
      } else if (mdOnly.value) {
        transformWidth = 205
      } else if (smOnly.value) {
        if (width < 668 && width > 374) {
          transformWidth = 122
        } else {
          transformWidth = 145
        }
      ...
      svg
        ...
        .attr('width', '100')
        .text(formatter.format(totalCompanyAccountNumber))
