...
export default {
  ...
  mounted() {
    if (Object.keys(this.$store.state.soloKOpenAccountDetail).length > 0) {
      this.addAccountHolderMode = this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag1
        ? this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag1
        : 'No'
    }
  },
  methods: {
    nextSlide() {
      this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag1 = this.addAccountHolderMode
      this.$emit('showAdditionalAccountForm1', this.addAccountHolderMode)
      //this.$emit('nextSlide')
    },
    prevSlide() {
      this.$emit('prevSlide')
