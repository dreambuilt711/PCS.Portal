...
export default {
  ...
  mounted() {
    if (Object.keys(this.$store.state.soloKOpenAccountDetail).length > 0) {
      this.addAccountHolderMode = this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag2
        ? this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag2
        : 'No'
    }
  },
  methods: {
    nextSlide() {
      this.$store.state.soloKOpenAccountDetail.AdditionalAccountFlag2 = this.addAccountHolderMode
      this.$emit('showAdditionalAccountForm2', this.addAccountHolderMode)
      //this.$emit('nextSlide')
    },
    prevSlide() {
      this.$emit('prevSlide')
