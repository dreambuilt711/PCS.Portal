        ...
        <v-col xl8 lg8 sm12 class="elevation-4" pa-3>
          <v-col xs12 sm12 md12 lg12 xl12>
            <v-col class="text-left">
              <h3>Click on login to access your TSM account.</h3>
            </v-col>
          </v-col>
          <v-col xs12 sm12 md12 lg12 xl12 class="text-right">
            <v-btn :color="$vuetify?.theme.dark ? '' : 'accent'" @click="initiateSSOLogin()"> Login </v-btn>
          </v-col>
        </v-col>
      </v-row>
    </v-col>
...
export default {
  methods: {
    initiateSSOLogin() {
      this.onPopUpEvent(`/api/v1/retention/tsm/sso/initiate`, '_TSM')
    }
  }
}
</script>
